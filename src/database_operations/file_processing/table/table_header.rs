...
impl BinarySerde for TableHeader {
    ...
    fn from_bytes(bytes: &[u8]) -> Result<Self, String> {
        ...
        let pages_per_file = u32::from_le_bytes(bytes[14..18].try_into().unwrap());
        let mut current_total: usize = 18;
...
mod tests {
    ...
    fn table_header_empty_columns() {
        let header = TableHeader::new(1, 0, 8, 1000, vec![]);
        let bytes = header.to_bytes();
        assert_eq!(bytes.len(), 18); // 8 (pages_count) + 2 (columns_count) + 4 (page_kbytes) + 4 (pages_per_file)
        let restored = TableHeader::from_bytes(&bytes).unwrap();
        assert_eq!(restored.to_bytes(), bytes);
    }
    ...
    fn table_header_too_short() {
        ...
        assert!(TableHeader::from_bytes(&[0; 17]).is_err());
    }
}
